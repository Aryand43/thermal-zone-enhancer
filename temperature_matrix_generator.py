...
def map_temperatures_from_lut(image_path: str, lut_path: str, save_path: str) -> np.ndarray:
    """
    Maps per-pixel temperatures from thermal imagery using RGB-to-temperature LUT
    with KDTree-based nearest-neighbour matching.

    Matches: Step 7 (concurrent) in invention. Produces °C matrix aligned with enhanced image.
    Novelty: Thermal-only mapping with option for ΔE-based precision filtering (see Step 3).
    """
    ...
    for idx, pixel_lab in enumerate(rgb_lab):
        ...
        if deltas[min_delta_idx] < 25:  # ΔE threshold
